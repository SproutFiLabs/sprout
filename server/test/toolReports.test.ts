...
describe("planning tools", () => {
  test("inflation adjusts purchasing-power targets and monthly contributions without inventing investment growth", () => {
    const report = goalReport({ initial: 0, monthly: 100, months: 12, annualReturn: 0, target: 1200, inflationRate: 10 });
    expect(report.targetAtEnd).toBe(1320);
    expect(report.requiredMonthly).toBe(110);
    expect(report.shortfall).toBe(120);
    expect(report.firstGoalMonth).toBeNull();
    expect(report.rows.at(-1)!.projected).toBe(1200);
    expect(goalReport({ initial: 0, monthly: 110, months: 12, annualReturn: 0, target: 1200, inflationRate: 10 }).firstGoalMonth).toBe(12);
  });
  test("inflation compounds over fractional years, supports old requests and rejects invalid rates", () => {
    const base = { initial: 0, monthly: 0, months: 6, annualReturn: 0, target: 1000 };
    expect(goalReport({ ...base, inflationRate: 10 }).targetAtEnd).toBe(1048.81);
    expect(goalReport(base)).toEqual(goalReport({ ...base, inflationRate: 0 }));
    for (const inflationRate of [-1, 21, NaN, Infinity]) expect(() => goalReport({ ...base, inflationRate })).toThrow();
  });
