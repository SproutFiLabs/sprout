...
import { memoryDb, testApp, readHeaders } from './helpers';
...
describe('history download', () => {
  test('requires a family session and rejects outsiders', async () => {
    const db = seed(); const app = testApp(db, ctx([]));
    expect((await app.request(`/api/sprouts/${VAULT}/history.csv`)).status).toBe(401);
    expect((await app.request(`/api/sprouts/${VAULT}/history.csv`, {headers: readHeaders(db, '0x0000000000000000000000000000000000000099')})).status).toBe(403);
    db.close();
  });
  test('lists every family event in whole units with its transaction', async () => {
    const calls: number[] = [];
    const db = seed();
    const app = testApp(db, ctx(calls));
    const res = await app.request(`/api/sprouts/${VAULT}/history.csv`, {headers: readHeaders(db, PARENT)});
  ...
  test('a value that looks like a formula is neutralized', async () => {
    ...
    const text = await (await app.request(`/api/sprouts/${VAULT}/history.csv`, {headers: readHeaders(db, PARENT)})).text();
    const withdrawal = text.split('\r\n').find((l) => l.includes(',Withdrawal,'))!;
    expect(withdrawal).toContain(`"'=HYPERLINK(""http://x"")"`);
  });

  test('unknown sprouts are unavailable to the session', async () => {
    const db = memoryDb();
    const app = testApp(db, ctx([]));
    expect((await app.request(`/api/sprouts/${VAULT}/history.csv`, {headers: readHeaders(db, PARENT)})).status).toBe(403);
  });
});
